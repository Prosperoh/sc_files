...
Pdef(\env, Penvir((), Ptpar([
    0.0, Pbind(
        ...
        \rq, Pdefn(\sounds_rq),
        \harmonicMovAmp, Pbrown(0.05, 0.08, 0.03, inf),
        \harmonicMov, 1.0,
        \octave, 3,
        \out, ~routerIn,
        \timingOffset, 0.1,
    ...
    0.1, Pbind(
        ...
        \timingOffset, Pkey(\dur) * Pwhite(0.1, 0.5),
        ...
        \amp, Pdefn(\sounds_amp),
        \ffreq, Pdefn(\sounds_ffreq) * 1.5,
        \rq, Pdefn(\sounds_rq) / 2.0,
        \harmonicMovAmp, Pexprand(3.0, 0.5, inf) * Pbrown(0.05, 0.08, 0.03, inf),
        \harmonicMov, Pfunc { ~lastEvent[\harmonicMov] },
        \octave, Pfunc { ~lastEvent[\octave] } + 1,
...
(
Pdefn(\sounds_harmonicAttenuation, 1.0);
